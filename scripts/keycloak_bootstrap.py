...
Description:
    ...
    backend/front-end clients, and initial users for Server Info Watchdog.
...
Usage:
    python scripts/keycloak_bootstrap.py \
        ...
        --realm watchdog \
        --frontend-client-id watchdog-frontend \
        --backend-client-id watchdog-backend \
        --frontend-root-url http://localhost:8080 \
        --api-root-url http://localhost:5000 \
        --user admin:admin:watchdog:admin

Granular Roles:
    - watchdog:admin - Full access (all permissions)
    - watchdog:read  - View-only access to monitoring data
"""
...
# Granular role definitions with descriptions for Watchdog
GRANULAR_ROLES = {
    "watchdog:read": "View monitoring data and status (LOW criticality)",
    "watchdog:admin": "Full access including configuration changes",
}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Bootstrap a Keycloak realm with clients, roles, and users for Server Info Watchdog.",
    ...
    parser.add_argument("--realm", default="watchdog", help="Realm name")
    parser.add_argument(
        "--frontend-client-id",
        default="watchdog-frontend",
        help="Frontend client ID",
    )
    parser.add_argument(
        "--backend-client-id",
        default="watchdog-backend",
